...
def _combine_issue_generator_debug_info(
    ...
    collected_debug_info: tuple[
        tuple[str, IssueIdentificationDebugInfo], ...
...
def run(
    ...
    logger.debug("[TIMING] PIPELINE: starting issue identification pipeline")
    ...
    ensure_global_resource_limits(
        max_dollars=(
            config.max_identifier_spend_dollars
            if config.max_identifier_spend_dollars is not None
            else float("inf")
        )
    )
    ...
    logger.debug(
        "[TIMING] PIPELINE: entire issue identification pipeline completed in {elapsed:.2f}s",
