...
def get_enabled_identifier_names(
    config: VetConfig,
        ...
        identifiers.append(
            (
                ...
                harness.make_issue_identifier(
                    identification_guides=tuple(guides_by_code[code] for code in issue_codes)
                ),
            )
        )
...
def run(
    ...
    identifiers = _build_identifiers(get_enabled_identifier_names(config), enabled_issue_codes, config.guides_by_code)
    ensure_global_resource_limits(
        max_dollars=(
