...
from imbue_core.agents.llm_apis.api_utils import create_costed_language_model_response_for_single_result
...
def _gemini_exception_manager() -> Iterator[None]:
    ...
    try:
        ...
        log_exception(
            e, "Failed to generate output from Gemini, unknown error of type {type_name}", type_name=type(e).__name__
        )
...
class GeminiAPI(LanguageModelAPI):
    ...
    async def _call_api(
        ...
        with _gemini_exception_manager():
            ...
            generation_config = GenerateContentConfig(
                ...
                thinking_config=fmap(
                    lambda thinking: ThinkingConfig(
                        thinking_budget=thinking.max_tokens, include_thoughts=thinking.output_thinking
                    ),
                    params.thinking,
                ),
            )
            ...
            thoughts_list = fmap(
                lambda content: fmap(lambda parts: [part.text for part in parts if part.thought], content.parts),
                candidate.content,
            )
            ...
            return create_costed_language_model_response_for_single_result(
                ...
                thoughts=fmap(lambda x: ThoughtResponse(text=x, completion_tokens=thought_tokens), thoughts),
            )
        ...
        with _gemini_exception_manager():
            ...
            result = CachedCountTokensResponse(
                response=CountTokensResponse(
                    input_tokens=total_tokens, cached_content_token_count=response.cached_content_token_count
                ),
                inputs=CountTokensInputs(model=self.model_info.model_name, prompt=text)
